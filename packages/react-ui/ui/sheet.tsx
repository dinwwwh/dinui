'use client'

import { cn } from '../utils'
import { button } from './button'
import * as ReactDialog from '@radix-ui/react-dialog'
import { X } from '@untitled-ui/icons-react'
import { cva, type VariantProps } from 'class-variance-authority'
import * as React from 'react'

export const Sheet = ReactDialog.Root

export const SheetTrigger = ReactDialog.Trigger

export const SheetClose = ReactDialog.Close

const sheetContent = cva(
  [
    'fixed z-50 gap-4 bg-white shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
    'border-gray-200',
  ],
  {
    variants: {
      side: {
        top: 'inset-x-0 top-0 border-b data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top',
        bottom:
          'inset-x-0 bottom-0 border-t data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom',
        left: 'inset-y-0 left-0 h-full w-3/4 border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-sm',
        right:
          'inset-y-0 right-0 h-full w-3/4  border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm',
      },
    },
    defaultVariants: {
      side: 'right',
    },
  },
)

interface SheetContentProps
  extends React.ComponentPropsWithoutRef<typeof ReactDialog.Content>,
    VariantProps<typeof sheetContent> {
  overlayProps?: React.ComponentProps<typeof ReactDialog.Overlay>
  portalProps?: React.ComponentProps<typeof ReactDialog.Portal>
}

export const SheetContent = React.forwardRef<
  React.ElementRef<typeof ReactDialog.Content>,
  SheetContentProps
>(({ portalProps, overlayProps, side = 'right', className, children, ...props }, ref) => {
  return (
    <ReactDialog.Portal {...portalProps}>
      <ReactDialog.Overlay
        {...overlayProps}
        className={cn(
          'fixed inset-0 z-50 bg-black/80  data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
          overlayProps?.className,
        )}
      />
      <ReactDialog.Content ref={ref} className={cn(sheetContent({ side }), className)} {...props}>
        {children}
      </ReactDialog.Content>
    </ReactDialog.Portal>
  )
})
SheetContent.displayName = ReactDialog.Content.displayName

interface SheetHeaderProps extends React.HTMLAttributes<HTMLDivElement> {
  closeButtonProps?: React.ComponentProps<typeof ReactDialog.Close>
}

export const SheetHeader = ({
  closeButtonProps,
  className,
  children,
  ...props
}: SheetHeaderProps) => {
  const CloseButton = () => {
    const { className, ...props } = closeButtonProps ?? {}
    return (
      <ReactDialog.Close
        className={cn(
          button({ size: 'icon:md', variant: 'tertiary-gray' }),
          'absolute top-3 right-4',
          className,
        )}
        {...props}
      >
        <X className="size-5" />
        <span className="sr-only">Close</span>
      </ReactDialog.Close>
    )
  }

  return (
    <div className={cn('relative flex flex-col gap-2 px-6 pt-6', className)} {...props}>
      <CloseButton />
      {children}
    </div>
  )
}
SheetHeader.displayName = 'SheetHeader'

export const SheetTitle = React.forwardRef<
  React.ElementRef<typeof ReactDialog.Title>,
  React.ComponentPropsWithoutRef<typeof ReactDialog.Title>
>(({ className, ...props }, ref) => (
  <ReactDialog.Title
    ref={ref}
    className={cn('text-xl font-semibold text-gray-900 pr-8', className)}
    {...props}
  />
))
SheetTitle.displayName = ReactDialog.Title.displayName

export const SheetDescription = React.forwardRef<
  React.ElementRef<typeof ReactDialog.Description>,
  React.ComponentPropsWithoutRef<typeof ReactDialog.Description>
>(({ className, ...props }, ref) => (
  <ReactDialog.Description
    ref={ref}
    className={cn('text-sm text-gray-600', className)}
    {...props}
  />
))
SheetDescription.displayName = ReactDialog.Description.displayName

export const SheetFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn('flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2', className)}
    {...props}
  />
)
SheetFooter.displayName = 'SheetFooter'
